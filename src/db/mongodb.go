...
func init() {
	log.Debug("init mongodb.....")
	db, err := Dial(config.GetConfig().MongoAddr, 100)
	if err != nil {
		log.Fatal("dial mongodb error: %v", err)
	}
	mongoDB = db
	if config.GetConfig().MongoAddr != config.GetConfig().GameMongo {
		gdb, err := Dial(config.GetConfig().GameMongo, 100)
		if err != nil {
			log.Fatal("dial mongodb error: %v", err)
		}
		gameDB = gdb
...
func initCollection() {
	...
	err = db.EnsureCounter(DB, "counters", "robotmatchnum")
	if err != nil {
		log.Fatal("ensure counter error: %v", err)
	}
	...
	err = db.EnsureCounter(DB, "counters", "horselampcontrol")
	if err != nil {
		log.Fatal("ensure counter error: %v", err)
	}
	err = db.EnsureCounter(DB, "counters", "activitycontrol")
	if err != nil {
		log.Fatal("ensure counter error: %v", err)
	}
